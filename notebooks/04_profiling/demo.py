...
### The work division strategies we'll compare
...
if __name__ == "__main__":
    ...
    tasks = {
        'cpu':   (cpu_task,   dict(n_batches=8, n_items=3_000_000)), 
        'io':    (io_task,    dict(n_batches=8, n_items=200)),
        'numpy': (numpy_task, dict(n_batches=8, n_items=1500)),
    }
    executors = {'serial': serial, 'threads': threaded, 'multiprocessing': multiprocess}

    times = run( executors[args.execution], tasks[args.task][0], **tasks[args.task][1])
